...
*/
print("AutoBuildThread main");
function main () {
	print("AutoBuildThread config init");
	Config.init(); // includes libs/common/AutoBuild.js

	print("AutoBuildThread variables");
	...
	print("AutoBuildThread array.prototype");
	// Will check if value exists in an Array
	Array.prototype.contains = function (val) { return this.indexOf(val) > -1; };

	print("AutoBuildThread functions");
	this.skillInValidRange = function(id) {
		switch (me.classid) {
	...
	this.gainedLevels = function() { return me.charlvl - prevLevel; };


	this.canSpendPoints = function() {
	...
	this.spendStatPoint = function(id) {
		var unusedStatPoints = me.getStat(4);
		if (SPEND_POINTS) {
			useStatPoint(id);
	...
	this.spendStatPoints = function() {
		...
		for (var i = 0; i < len; i++) {
			...
			if (id === -1) { continue; }
				...
				var pointSpent = this.spendStatPoint(id);
				if (SPEND_POINTS) {
					if (!pointSpent) {
	...
	this.getRequiredSkills = function(id) {
		...
		this.searchSkillTree = function(id) {
			var results = [];
		...
		var requirements = [];
		this.searchSkillTree(id);
		function increasing (a, b) { return a-b; };
		return requirements.sort(increasing);
	};


	this.spendSkillPoint = function(id) {
		var unusedSkillPoints = me.getStat(5);
		var skillName = this.getSkillById(id)+" ("+id+")";		// TODO: Use let ?
		if (SPEND_POINTS) {
			this.useSkillPoint(id);
	...
	this.spendSkillPoints = function() {
		if (AutoBuildTemplate[me.charlvl] === undefined) {
			return true;
		}
		...
		for (var i = 0; i < len; i++) {
			...
			var requiredSkills = this.getRequiredSkills(id);
			...
			var pointSpent = this.spendSkillPoint(id);
	...
	try {
		...
		while (true) {
			...
			if (levels > 0 && (canSpendPoints() || Config.AutoSkill.Enabled || Config.AutoStat.Enabled)) {
				...
				print("autobuildthread Level up detected (", prevLevel, "-->", me.charlvl, ")");
